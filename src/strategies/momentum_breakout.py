...
class MomentumBreakout(BaseStrategy):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.trail_pct = config.get("trail_pct", 0.01)       # 고점 대비 1% 하락 시 청산
        self.stop_loss_pct = config.get("stop_loss_pct", 0.02) # 진입가 대비 2% 손절
        self.min_hold_ticks = config.get("min_hold_ticks", 2)   # 최소 보유 틱 (즉시 청산 방지)
    ...
    def risk_limits(self) -> Dict[str, float]:
        return {"max_loss_per_trade": self.stop_loss_pct}
